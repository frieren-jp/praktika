...
## BASIC-3 - ревью и демо ToDo API

Задача: подготовиться к созвону/встрече с руководителем по мини-проекту BASIC-2.

Формат: демо 5 минут, code review 10 минут, рефлексия 5 минут.

В папке `basic3` лежат:

- `README.md` - что открыть перед встречей;
- `review-prep.md` - план демо, ответы на ожидаемые вопросы и идеи улучшений.

Проверено:

- проект `todo_app` запускается в отдельном чистом окружении `.venv-basic3-check`;
- команда `.\check.ps1` проходит успешно;
- все 5 эндпоинтов отвечают;
- временный venv, база и логи удалены после проверки.

Главные вопросы для подготовки:

- почему зависимости объявлены аннотациями;
- что дают `layers`;
- зачем нужен `UnitOfWork`;
- как archtool связывает `UserRepoABC` и `UserRepo`;
- как добавить новый модуль `notifications`;
- что можно улучшить в `archtool` и `web_fractal`.

Статус: материалы для ревью подготовлены.
